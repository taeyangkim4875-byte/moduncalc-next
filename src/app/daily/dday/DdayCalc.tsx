...
export default function DdayCalc(){
  ...
  return(<>
    <Card><SectionTitle num="📅">D-day 계산</SectionTitle>
      ...
      {dd&&<ResultPanel className="mt-3" value={dd.diff>0?`D-${dd.diff}`:dd.diff<0?`D+${dd.abs}`:'D-Day'} sub={`${dd.abs}일 (${dd.w}주 ${dd.r}일) · ${dd.date} (${dd.day}요일)`}/>}
    </Card>
    <Card><SectionTitle num="📆">날짜 더하기/빼기</SectionTitle>
      ...
      <ResultPanel className="mt-3" value={da.date} sub={`${da.day}요일`}/>
    </Card>
    <Card>
