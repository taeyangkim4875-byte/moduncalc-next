...
export default function SpeedCalc(){
  ...
  return(<>
    <Card><SectionTitle num="🚗">속도 · 거리 · 시간</SectionTitle>
      ...
      <ResultPanel className="mt-3" value={speed?`${hr}시간 ${mn}분`:'—'} sub={`${fmt(dist)}km ÷ ${fmt(speed)}km/h`}/>
    </Card>
    <Card><SectionTitle num="🏃">속도 단위 변환</SectionTitle>
      ...
      <div className="grid grid-cols-2 gap-2.5"><ResultPanel value={fmt(conv/3.6)} sub="m/s"/><ResultPanel value={fmt(conv*0.621371)} sub="mph"/></div>
    </Card>
    <Card>
      <h2 className="text-base font-extrabold mb-3">이 계산기는요</h2>
      <p className="text-sm text-[#4E5968] leading-relaxed">거리랑 속도 넣으면 소요 시간 바로 나와요. 속도 단위 변환(km/h, m/s, mph)도 할 수 있어요.</p>
    </Card>
    <Card>
