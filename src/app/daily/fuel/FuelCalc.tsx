...
export default function FuelCalc(){
  ...
  return(<>
    <Card><SectionTitle num="⛽">자동차 연비 계산</SectionTitle>
      ...
      <ResultPanel className="mt-3" value={`${fmt(eff1)} km/L`} sub={`${fmt(dist)}km ÷ ${fmt(liter)}L`}/>
    </Card>
    <Card><SectionTitle num="💰">여행 유류비 계산</SectionTitle>
      ...
      <div className="grid grid-cols-2 gap-2.5 mt-3">
        <ResultPanel className="mt-3" value={`${fmt(liters2)}L`} sub="예상 주유량"/>
        <ResultPanel className="mt-3" value={`${Math.round(cost).toLocaleString()}원`} sub="예상 유류비"/>
      </div>
    </Card>
    {/* 전기차 전비 계산 */}
    <Card><SectionTitle num="🔋">전기차 전비 계산</SectionTitle>
      ...
      <div className="grid grid-cols-2 gap-2.5 mt-3">
        <ResultPanel className="mt-3" value={`${fmt(evKwh)} kWh`} sub="예상 충전량"/>
        <ResultPanel className="mt-3" value={`${Math.round(evCost).toLocaleString()}원`} sub="예상 충전비"/>
      </div>
    </Card>
