  ...
  return(<>
    <Card><SectionTitle num="💰">할인가 계산</SectionTitle>
      ...
      <div className="grid grid-cols-2 gap-2.5 mt-3">
        <ResultPanel value={`${Math.round(price*(1-rate/100)).toLocaleString()}원`} sub="할인 적용가"/>
        <ResultPanel value={`${Math.round(price*rate/100).toLocaleString()}원`} sub={`할인 금액 (${rate}%)`}/>
      </div>
    </Card>
    <Card><SectionTitle num="🔢">1+1 / 2+1 단가 비교</SectionTitle>
      ...
      <div className="grid grid-cols-2 gap-2.5 mt-3">
        <ResultPanel value={`${Math.round(bp/2).toLocaleString()}원`} sub="1+1 개당"/>
        <ResultPanel value={`${Math.round(bp/3).toLocaleString()}원`} sub="2+1 개당"/>
      </div>
    </Card>
    <Card>
