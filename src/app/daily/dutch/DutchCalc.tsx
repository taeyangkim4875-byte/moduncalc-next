...
export default function DutchCalc(){
  ...
  return(<>
    <Card><SectionTitle num="🍽️">N분의1 더치페이</SectionTitle>
      ...
      <ResultPanel className="mt-3" value={`${people>0?Math.ceil(total/people).toLocaleString():0}원`} sub={`1인당 (${people}명)`}/>
    </Card>
    <Card><SectionTitle num="💸">팁 계산기</SectionTitle>
      ...
      <div className="grid grid-cols-2 gap-2.5 mt-3">
        <ResultPanel value={`${Math.round(tipTotal*tipRate/100).toLocaleString()}원`} sub={`팁 (${tipRate}%)`}/>
        <ResultPanel value={`${Math.round(tipTotal*(1+tipRate/100)).toLocaleString()}원`} sub="총 결제액"/>
      </div>
    </Card>
    <Card>
